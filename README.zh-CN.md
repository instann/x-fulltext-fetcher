...
> 一个只读研究工具、feed 生成器、前端仪表盘和 Codex Skill，用来把公开的 X/Twitter 链接转换成干净的研究笔记。

X 全文抓取器可以帮助你收集公开推文和 X Article，适合商业研究、AI 研究、市场观察、创作者内容分析等工作流。它像一个轻量级的 MCP 风格提取层：输入公开 status 链接或来源列表，输出 Markdown 笔记、digest 文件、元数据、JSON Feed、RSS，以及可选的原始 JSON，方便后续索引和分析。
...
- 按需保存元数据和原始 JSON。
- 生成本地 digest、JSONL 索引、JSON Feed 和 RSS。
- 在内置静态仪表盘里浏览收集结果。
